...
const Index = () => {
  ...
  const {
    ...
    staleTime: 0,
    // Set staleTime to 0 to always get fresh data
  ...
  const filteredEvents = events.filter(event => {
    ...
    if (searchQuery) {
      ...
      if (!event.title.toLowerCase().includes(query) && !event.venue.toLowerCase().includes(query) && !event.date.toLowerCase().includes(query) && !(event.location || "").toLowerCase().includes(query)) {
        return false;
      }
    }
  ...
  <div className="text-center space-y-2">
    ...
    <p className="text-sm text-muted-foreground/70 animate-fade-in my-[9px]">
      {(() => {
