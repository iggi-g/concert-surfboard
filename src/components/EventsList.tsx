...
export const EventsList = ({ events, isLoading, showFavoritesOnly = false, onVenueClick, onDateClick }: EventsListProps) => {
  ...
  if (isLoading) {
    return (
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 md:gap-6 w-full max-w-[1920px] mx-auto px-4">
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 md:gap-6 w-full max-w-[1920px] mx-auto px-4">
      {filteredEvents.map((event: Event) => (
        <div key={`${event.title}-${event.date}`} className="flex justify-center" style={{ contentVisibility: 'auto', containIntrinsicSize: '0 480px' }}>
          <ConcertCard
            artist={event.title}
            date={event.date}
            venue={event.venue}
